...
namespace InstantLoot.HelperFunctions
{
	public static class ScrapHelperFunctions
	{
		...
		private static void ReportScrapList(string where)
		{
			if (where == "ship")
			{
				...
				shipObjects.Do(scrap => InstantLoot.Log($"{scrap.name} - ${scrap.scrapValue} - ${scrap.targetFloorPosition.x}- ${scrap.targetFloorPosition.y}- ${scrap.targetFloorPosition.z}"));
			}
			else if (where == "closet")
			{
				StorageClosetHelper sch = new();
				var closetObjects = sch.GetObjectsInStorageCloset();
				closetObjects.Do(scrap => InstantLoot.Log($"{scrap.name} - ${scrap.scrapValue} - ${scrap.targetFloorPosition.x}- ${scrap.targetFloorPosition.y}- ${scrap.targetFloorPosition.z}"));
			}
		}
